...
public class GeneralizationCreateCommand extends CreateElementCommand {
	...
	public boolean canExecute() {
		...
		return OntoUML.diagram.edit.policies.OntoUMLBaseItemSemanticEditPolicy.LinkConstraints
				.canCreateGeneralization_4006(getContainer(), getSource(),
						getTarget());
	}
	...
	protected EObject doDefaultElementCreation() {
		OntoUML.Generalization newElement = OntoUML.OntoUMLFactory.eINSTANCE
				.createGeneralization();
		getContainer().getInstanciarElement().add(newElement);
		newElement.getTarget().add(getSource());
		newElement.getSource().add(getTarget());
		return newElement;
	}
